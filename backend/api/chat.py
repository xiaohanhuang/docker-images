...
async def pin_widget(request: Request, body: dict[str, Any]):
    ...
    store = get_store()
    await store.set(_PINS_PREFIX, pin_id, pinned.model_dump())
...
async def list_pins(request: Request) -> list[dict[str, Any]]:
    ...
    user = getattr(request.state, "user", "unknown")
    store = get_store()
    return await store.list_by_field(_PINS_PREFIX, "user", user)
...
async def unpin_widget(pin_id: str, request: Request):
    ...
    user = getattr(request.state, "user", "unknown")
    store = get_store()
    pin = await store.get(_PINS_PREFIX, pin_id)
    if not pin or pin["user"] != user:
        raise HTTPException(status_code=404, detail="Pin not found")
    await store.delete(_PINS_PREFIX, pin_id)
