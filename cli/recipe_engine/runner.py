...
class RecipeRunner:
    ...
    def _name_variants(name: str) -> list[str]:
        ...
        Components with tasks extracted to ``_task.py`` are registered as
        ``pkg.foo._task.func``.  This helper yields all variants so the runner
        ...
        if len(parts) >= 2:
            ...
            # foo.func → foo._task.func
            task_name = f"{prefix}.{pkg}._task.{func}" if prefix else f"{pkg}._task.{func}"
            if task_name != name:
            ...
            # foo._task.func → foo.func  (reverse)
            if pkg == "_task" and len(parts) >= 3:
    ...
    def resolve_config(
        ...
        if preset_name:
            ...
            first_profile = sorted(recipe.infrastructure.profiles.keys())[0]
            config = {
    ...
    def run(
        ...
        try:
            ...
            for step_idx, step in enumerate(ordered_steps):
                    ...
                    if isinstance(v, str):
                        ...
                        if m:
                            ...
                            if key.startswith("steps."):
                                # "steps.load_dataset.outputs.o0" -> bind to node Promise
                                ref_parts = key.split(".")
                                ref_node_name = ref_parts[1]
                                ref_out = ref_parts[3] if len(ref_parts) > 3 else "o0"
                                node_inputs[k] = ordered_nodes[ref_node_name].outputs[ref_out]
                                continue
                ...
                # Add the remote entity as a DAG node
                ...
                if step_idx == len(ordered_steps) - 1 and len(node.outputs) > 0:
                    output_key = list(node.outputs.keys())[0]  # output is usually o0
                    ...
                    if (
                        ...
                        try:
                            ...
                            pass
                    wf.add_workflow_output("o0", node.outputs[output_key], python_type=out_py_type)
            ...
            execution = remote.execute(
                ...
                project=proj,
                domain=dom,
                version=f"v1-recipe-{os.urandom(4).hex()}",
            )
