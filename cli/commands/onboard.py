...
def generate_llm_finetune_project(project_name: str, base_dir: Path) -> Path:
    ...
    workflow_py = textwrap.dedent('''\
        ...
        TRAINING_IMAGE = (
            "{{ECR_REGISTRY}}/{{ECR_REPO}}/training-llm:latest"
        )
    ...
    config_yaml = textwrap.dedent(f"""\
        ...
        image: {{{{ECR_REGISTRY}}}}/{{{{ECR_REPO}}}}/training-llm:latest
        resources:
          head: {{cpu: "4", mem: "16Gi"}}
...
def generate_rag_pipeline_project(project_name: str, base_dir: Path) -> Path:
    ...
    workflow_py = textwrap.dedent('''\
        ...
        DATA_IMAGE = "{{ECR_REGISTRY}}/{{ECR_REPO}}/data-cpu:latest"
        GENAI_IMAGE = "{{ECR_REGISTRY}}/{{ECR_REPO}}/genai-gpu:latest"
...
def generate_distributed_training_project(project_name: str, base_dir: Path) -> Path:
    ...
    workflow_py = textwrap.dedent('''\
        ...
        ML_IMAGE = "{{ECR_REGISTRY}}/{{ECR_REPO}}/ml-gpu:latest"
