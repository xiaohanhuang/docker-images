    ...
    try:
        ...
        if packages:
            if not isinstance(packages, (list, tuple)):
                return (
                    jsonify({"error": "'packages' must be a list or tuple of strings"}),
                    400,
                )
            ...
            for pkg in packages:
                if not isinstance(pkg, str):
                    return (
                        jsonify({"error": "All 'packages' entries must be strings"}),
                        400,
                    )
                if not re.fullmatch(r"[a-zA-Z0-9][a-zA-Z0-9_\-\.\=\>\<]*", pkg):
