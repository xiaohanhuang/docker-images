"""
ML Platform Execution Service (v0.2.0).
...
Includes pod pool support for warm container reuse to reduce cold-start latency.
"""
...
try:
    ...
    print("[execution] Running in-cluster, loaded service account credentials")
except config.ConfigException:
    try:
        config.load_kube_config()
        print("[execution] Running locally, loaded kubeconfig")
    except config.ConfigException as e:
        print(f"[execution] ❌ Could not load Kubernetes config: {e}")
        raise SystemExit("[execution] Failed to load Kubernetes config, exiting") from e
...
# Default image tags for execution pods.
# Each image has its own version — bump independently when rebuilt.
# Tags must match what's published in ECR (see projects/components/images/).
DEFAULT_BASE_GPU_TAG = "latest"
DEFAULT_BASE_CPU_TAG = "latest"
DEFAULT_POOL_GPU_TAG = "latest"
DEFAULT_POOL_CPU_TAG = "latest"
...
# Create FastAPI app
app = FastAPI(
    title="ML Platform Execution Service",
...
async def root():
    ...
    return {
        "service": "ml-platform-execution-service",
        "version": "0.1.0",
        "status": "healthy",
    }
...
async def execute_remote(
    ...
    try:
        ...
        return Response(
            content=f"Failed to parse JSON request: {e}",
            status_code=400,
        )

    fn_name = req_json.get("fn_name")
    config = req_json.get("config", {})
    payload_b64 = req_json.get("payload")

    if not fn_name or not payload_b64:
        return Response(content="Missing 'fn_name' or 'payload' in request", status_code=400)

    try:
        payload_bytes = base64.b64decode(payload_b64)
    except Exception as e:
        return Response(
            content=f"Failed to decode base64 payload: {e}",
            status_code=400,
        )
    ...
    print("[execution] New execution request")
    print(f"[execution] Function: {fn_name}")
    print(f"[execution] Config hash: {config_hash}")
    print(f"[execution] Namespace: {namespace}")
    print(f"[execution] Pool enabled: {POD_POOL_ENABLED}")
        ...
        async with pool_lock:
            ...
            for name, state in list(pod_pool.items()):
                ...
                try:
                    ...
                    if exc.status == 404:
                        print(f"[execution] Pooled pod {name} no longer exists; removing")
                        pod_pool.pop(name, None)
                        continue
                    print(f"[execution] Error reading pod {name}: {exc}")
                    continue
                ...
                if pod_phase != "Running" or not current_ip:
                    print(
                        f"[execution] Pooled pod {name} not ready "
                        f"(phase={pod_phase}, ip={current_ip}); skipping"
                    )
                    continue

                # Found a matching, healthy idle pod
                print(f"[execution] ♻️  Reusing warm pod: {name} with IP: {current_ip}")
    ...
    if new_pod_needed:
        ...
        print(f"[execution] Creating new pod: {execution_id}")
        ...
        if not use_pool:
            try:
                ...
                print(f"[execution] Created Job: {execution_id}")
            except Exception as e:
                print(f"[execution] Failed to create job: {e}")
                return Response(
    ...
    if use_pool:
            ...
            try:
                if _new_pod_needed:
                    ...
                    try:
                        ...
                        yield f"[execution] Failed to create pool pod: {e}\n".encode()
                        return

                    yield (
                        f"[execution] Pool pod {_pod_name} created," " waiting for ready...\n"
                    ).encode()
                    ...
                    while wait_secs < 600:
                        ...
                        line = f"\r\033[K[execution] Waiting for pod... {spin}"
                        yield line.encode()
                        await asyncio.sleep(1)
                        wait_secs += 1
                    else:
                        yield "\n[execution] Timeout waiting for pool pod to be ready\n".encode()
                        return

                    yield f"\n[execution] Pool pod ready with IP: {_pod_ip}\n".encode()
                    ...
                    yield f"[execution] ♻️  Reusing warm pod: {_pod_name}\n".encode()

                # Execute on pod
                yield "[execution] Sending payload to executor...\n".encode()
                try:
                    ...
                    async with pool_lock:
                        if _pod_name in pod_pool:
                            pod_pool[_pod_name].status = "idle"
                    yield f"[execution] Execution failed: {e}\n".encode()

            except Exception as e:
                yield f"[execution] Pool streamer error: {e}\n".encode()
        ...
        async def log_streamer():
            ...
            try:
                ...
                if not job_pod_name:
                    yield "[execution] Timeout waiting for pod to be created\n".encode()
                    return

                yield f"[execution] Pod {job_pod_name} created, waiting for logs...\n".encode()
                ...
                try:
                    ...
                    print(f"[execution] Cleaned up Job: {execution_id}")
                except Exception as e:
                    print(f"[execution] Failed to cleanup Job: {e}")
...
async def _create_executor_pod(
    ...
    if config.get("gpu", 0) > 0:
        default_image = f"ml-platform/executor-pool:{DEFAULT_POOL_GPU_TAG}"
    else:
        default_image = f"ml-platform/executor-pool-cpu:{DEFAULT_POOL_CPU_TAG}"
    user_image = config.get("image")
    if user_image:
        print(
            f"[execution] ⚠️  Custom image '{user_image}' in pool mode — "
                ...
                if pod.status.conditions:
                    ...
                    for condition in pod.status.conditions:
                        if condition.type == "Ready" and condition.status == "True":
                            ...
                            break
                    if ready:
                        print(f"[execution] Pod {pod_name} is ready with IP {pod.status.pod_ip}")
                        return pod.status.pod_ip

        except Exception as e:
            print(f"[execution] Error checking pod readiness: {e}")

        await asyncio.sleep(2)

    print(f"[execution] Timeout waiting for pod {pod_name} to be ready")
    return None
...
async def _execute_on_pod(
    ...
    try:
        ...
        if result.get("success"):
            output = stdout_log
            output += "[execution] Execution completed on warm pod\n"
            output += f"__RESULT_START__\n{result_b64}\n__RESULT_END__\n"
            return output
        else:
            output = stdout_log
            output += "[execution] Execution failed on warm pod\n"
            output += f"__RESULT_START__\n{result_b64}\n__RESULT_END__\n"
            return output
...
def _create_execution_job(
    ...
    if config.get("gpu", 0) > 0:
        default_image = f"ml-platform/base-gpu:{DEFAULT_BASE_GPU_TAG}"
    else:
        default_image = f"ml-platform/base-cpu:{DEFAULT_BASE_CPU_TAG}"
    ...
    while time.time() < deadline:
        try:
            ...
            if pods.items:
                ...
                print(f"[execution] Found pod: {pod_name}")
                return pod_name
        except Exception as e:
            print(f"[execution] Error listing pods: {e}")
...
async def _stream_pod_logs(pod_name: str, namespace: str):
    ...
    while time.time() < log_deadline:
        try:
            ...
            print(f"[execution] Error reading pod status: {e}")
    ...
    while time.time() < log_deadline:
        try:
            ...
            try:
                ...
                print(f"[execution] Streaming failed, will retry: {stream_err}")
                raise stream_err
            ...
            if e.status == 400:
                ...
                await asyncio.sleep(2)
                yield "[execution] Waiting for pod to start...\n"
                continue
            else:
                yield f"[execution] Error streaming logs: {e}\n"
                return
        except Exception:
            # Retry on other errors (transient network issues, etc.)
            await asyncio.sleep(2)
            yield "[execution] Log stream interrupted, retrying...\n"
            continue

    # Timeout waiting for logs
    yield "[execution] Timed out waiting for pod logs.\n"
    ...
    while time.time() < deadline:
        try:
            ...
            if job.status.succeeded:
                print(f"[execution] Job {execution_id} succeeded")
                return
            if job.status.failed:
                print(f"[execution] Job {execution_id} failed")
                return

        except Exception as e:
            print(f"[execution] Error reading job status: {e}")

        await asyncio.sleep(5)

    print(f"[execution] Job {execution_id} timed out after {timeout}s")
...
async def _ttl_eviction_task():
    ...
    """
    print(f"[execution] Starting TTL eviction task (TTL={POD_POOL_TTL_SECONDS}s)")
        ...
        try:
                ...
                for pod_name, state in pod_pool.items():
                    if state.status == "idle":
                        ...
                        if idle_seconds > POD_POOL_TTL_SECONDS:
                            print(
                                f"[execution] Pod {pod_name} idle for {idle_seconds:.0f}s "
            ...
            for pod_name in pods_to_evict:
                try:
                    ...
                    print(f"[execution] Evicted pod {pod_name}")
                    ...
                    print(f"[execution] Failed to evict pod {pod_name}: {e}")

        except Exception as e:
            print(f"[execution] Error in TTL eviction task: {e}")
...
async def _reconcile_pool_on_startup():
    ...
    label_selector = "app=remote-execution-pool,managed-by=execution-service"
    ...
    try:
        ...
        print(f"[execution] Startup reconcile: could not list pool pods: {e}")
        return
    ...
    for pod in pods.items:
        ...
        if phase == "Running" and ip:
            ...
            recovered += 1
            print(
                f"[execution] Startup: re-registered pool pod {name}"
                f" (hash={config_hash}, ip={ip})"
            )
        else:
            try:
                ...
                deleted += 1
                print(f"[execution] Startup: deleted orphaned pod {name} (phase={phase})")
            except Exception as e:
                print(f"[execution] Startup: could not delete orphaned pod {name}: {e}")

    if recovered or deleted:
        print(f"[execution] Startup reconcile: {recovered} re-registered, {deleted} deleted")
...
async def startup_event():
    ...
    if POD_POOL_ENABLED:
        print(f"[execution] Pod Pool enabled with TTL={POD_POOL_TTL_SECONDS}s")
        asyncio.create_task(_ttl_eviction_task())
        asyncio.create_task(_reconcile_pool_on_startup())
    else:
        print("[execution] Pod Pool disabled, using legacy one-shot Jobs")


if __name__ == "__main__":
    port = int(os.getenv("PORT", "8080"))
    print(f"[execution] Starting ML Platform Execution Service on port {port}")
