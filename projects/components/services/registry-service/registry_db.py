...
Server-side recipe registry used by the registry-service.
"""
...
-- Migrate existing tables that pre-date the profile_statuses column.
ALTER TABLE recipe_registry ADD COLUMN IF NOT EXISTS
    profile_statuses JSONB DEFAULT '{}';
"""
...
class RecipeRegistry:
    ...
    def __init__(
        ...
        self._pg_host = pg_host or os.getenv("POSTGRES_HOST", "postgres.postgres.svc.cluster.local")
        self._pg_port = pg_port or int(os.getenv("POSTGRES_PORT", "5432"))
        self._pg_dbname = pg_dbname or os.getenv("POSTGRES_DB", "flyte")
        self._pg_user = pg_user or os.getenv("POSTGRES_USER", "flyte")
        self._pg_password = pg_password or os.getenv("POSTGRES_PASSWORD", "")
        self._conn = None
    ...
    def get_verification_status(
        ...
        if row is None:
            return None
        # Per-profile statuses take priority over legacy single-status field.
        profile_statuses: Dict[str, str] = row.get("profile_statuses") or {}
        if profile in profile_statuses:
            return profile_statuses[profile]
        # Legacy fallback.
    ...
    def set_verification_status(
        ...
        profile_statuses: Dict[str, str] = dict(row.get("profile_statuses") or {})
