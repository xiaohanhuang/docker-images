...
def remote(
    ...
    agent_url: Optional[str] = None,  # deprecated, use execution_url
) -> Callable:
    """
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            return _execute_remote(
                ...
                spot=spot,
                ttl=ttl,
                execution_url=execution_url or agent_url,
            )
...
def _execute_remote(
    ...
    **config,
    ...
    execution_url = (
        config.get("execution_url")
        or config.get("agent_url")
        or os.getenv("ML_PLAT_EXECUTION_URL")
        or os.getenv("ML_PLAT_AGENT_URL")
    )
